...
# Period-shaped string values (e.g. "Apr-24", "Q1 2024", "2024-Q1", "2025").
# Used as a fallback when pd.to_datetime can't parse them but the column is
# semantically a time axis.
...
def _values_look_like_periods(series: pd.Series) -> bool:
    """True when most non-null string values match a known period shape."""
    sample = series.dropna().astype(str).head(20)
    if sample.empty:
        return False
    matches = sum(
        1 for v in sample
        if any(p.match(v.strip()) for p in _PERIOD_VALUE_PATTERNS)
    )
