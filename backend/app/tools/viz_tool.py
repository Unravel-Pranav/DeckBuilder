...
# Patterns that indicate a categorical column is actually a time period
# (e.g. "Q1 2024", "Apr-24", "2024-Q1", "Jan", "2025"). Treated as temporal
# so monthly/quarterly CSVs without an explicit datetime column still get
# line/multi-line charts.
_PERIOD_PATTERNS = [
    re.compile(r"^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[\s-]?\d{0,4}$", re.I),
    re.compile(r"^q[1-4][\s-]?\d{2,4}$", re.I),
...
def _looks_like_period_column(col: ColumnProfile) -> bool:
    """Detect period-style columns (months, quarters, years).

    Accepts categorical OR text typed columns — the profiler classifies a
    column as ``text`` when every value is unique (e.g. 12 unique month
    labels), even though semantically it's a time period.
    """
    if col.data_type not in ("categorical", "text") or not col.sample_values:
        return False
    matches = sum(
        1 for v in col.sample_values
        if any(p.match(str(v).strip()) for p in _PERIOD_PATTERNS)
    )
    return matches >= max(1, len(col.sample_values) // 2)
...
def _recommend_from_profile(profile: DataProfile) -> VizRecommendation:
    ...
    # Promote period-style columns (e.g. "Apr-24", "Q1 2024") to temporal
    # for the purposes of chart selection. Looks at categorical AND text
    # columns since fully-unique label sets get typed as text.
    period_cols = [
        c for c in profile.columns
        if c.data_type in ("categorical", "text") and _looks_like_period_column(c)
    ]
    effective_temporal = temporal_cols + period_cols
    is_effective_time_series = bool(effective_temporal) and bool(numeric_cols)

    n_cat = len(categorical_cols) - len([c for c in period_cols if c.data_type == "categorical"])
    n_num = len(numeric_cols)
    cat_distinct_max = max(
        (
            c.stats.distinct_count for c in categorical_cols
            if c.stats and c not in period_cols
        ),
        default=0,
    )

    if (patterns.is_time_series and temporal_cols) or is_effective_time_series:
        if n_num == 1:
